...
> **Status**: Currently Linux only. Windows support is in progress on the `main` branch.
...
3. **Wrapper Script** — Generates `~/.local/bin/claude-xxx` launcher scripts with per-profile `--settings` files
