...
3. **Wrapper 脚本** — 生成 `~/.local/bin/claude-xxx` 可执行脚本，设置环境变量指向本地代理
...
- **Wrapper 脚本生成** — 一键生成 `claude-xxx` 命令脚本到 `~/.local/bin/`
- **SSE 流式响应** — 支持流式输出透传
...
构建产物位于 `src-tauri/target/release/clair`。
...
选中 Profile 后，点击右侧 **Regenerate Wrapper**，生成 `~/.local/bin/claude-glm`。
...
- **配置数据库**: `~/.config/clair/clair.db` (SQLite)
- **日志目录**: `~/.config/clair/logs/`
- **Wrapper 脚本**: `~/.local/bin/claude-*`
