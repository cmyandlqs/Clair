...
#[cfg(test)]
mod tests {
    use super::{build_profile_settings_content, build_unix_wrapper_content, normalize_path_for_compare, paths_match, wrapper_settings_path};
    use std::path::Path;

    #[test]
    fn unix_wrapper_uses_settings_flag() {
        let settings_path = wrapper_settings_path(Path::new("/home/user/.local/bin"), "claude-glm").unwrap();
        let content = build_unix_wrapper_content("/usr/local/bin/claude", &settings_path);
